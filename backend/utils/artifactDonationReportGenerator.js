...
// Generate Artifact Donation Report HTML WITHOUT base64 images (for database storage)
async function generateArtifactDonationReportPDFSimple(report) {
  ...
  if (report.data) {
    try {
      ...
      console.error('Error parsing artifact donation report data:', e);
    }
  }

  // Filter to only artifact donations
  let donations = (reportData.donations || []).filter(d => d.type === 'artifact' || d.type === 'donated');
...
<head>
    <meta charset="UTF-8">
    <title>Donation Artifact List - Cagayan de Oro City Museum</title>
    <style>
...
<body>
    ...
    <div class="report-title">Donation Artifact List</div>
    ...
    <table class="donation-table">
        ...
        <tbody>
            ${donations && donations.length > 0 ? 
              ...
              `).join('') : 
              '<tr><td colspan="4" style="text-align: center; font-style: italic;">No artifact donations found for the selected period.</td></tr>'
            }
        </tbody>
    </table>
</body>
...
// Generate Artifact Donation Report PDF (WITH base64 images for PDF)
async function generateArtifactDonationReportPDF(report) {
  // Parse the report data
  let reportData = {};
  if (report.data) {
    try {
      reportData = JSON.parse(report.data);
      console.log('📊 Artifact donation report data keys:', Object.keys(reportData));
    } catch (e) {
      console.error('Error parsing artifact donation report data:', e);
    }
  }
  ...
  console.log('🎯 Processing artifact donations:', donations.length);
...
module.exports = {
  generateArtifactDonationReportPDF, // Full version with base64 images for PDF
  generateArtifactDonationReportPDFSimple // Simple version without base64 for database
};
