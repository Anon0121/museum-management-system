...
async function removeArchiveIdFromImages() {
  let connection;
  ...
  try {
    ...
    const sqlPath = path.join(__dirname, '../database/remove_archive_id_from_images.sql');
    ...
    console.log('✅ Archive ID column removed from images table successfully!');
    ...
    // If foreign key doesn't exist, that's okay - continue
    if (error.code !== 'ER_CANT_DROP_FIELD_OR_KEY' && error.code !== 'ER_DROP_INDEX_FK') {
      process.exit(1);
    } else {
      console.log('⚠️ Some constraints may not exist, but continuing...');
    }
  } finally {
    if (connection) {
