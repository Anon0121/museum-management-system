...
Focus on practical insights that would be valuable for museum management and operations.`;
  ...
  buildChatPrompt(message, conversationHistory, currentData) {
    const context = `
You are an expert museum management AI assistant for the Cagayan de Oro City Museum - Heritage Studies Center. You have access to the museum's database and system records only.

CURRENT MUSEUM SYSTEM DATA (Last 30 Days):
- Total Visitors: ${currentData.recentVisitors}
- Total Events: ${currentData.recentEvents}
- Total Donations: ${currentData.recentDonations.count} donations ($${currentData.recentDonations.amount.toFixed(2)})
- Cultural Objects: ${currentData.culturalObjects || 0}
- Archive Files: ${currentData.archives || 0}

AVAILABLE REPORT TYPES:
- visitor_analytics: Complete visitor behavior, demographics, trends, and entry patterns
- monthly_summary: Comprehensive monthly overview of all museum activities
- event_performance: Event attendance, success metrics, and effectiveness analysis
- financial_report: Donation trends, revenue analysis, and financial insights
- exhibit_analytics: Cultural object popularity, visitor engagement, and exhibit performance
- staff_performance: Staff productivity, visitor processing, and efficiency metrics

MUSEUM IMPROVEMENT CAPABILITIES:
- Visitor engagement strategies based on current data
- Event planning suggestions using visitor trends and patterns
- Exhibit optimization recommendations
- Staff training and performance improvement suggestions
- Financial growth strategies based on donation patterns
- Marketing and outreach recommendations

USER MESSAGE: ${message}

CONVERSATION HISTORY: ${conversationHistory.map(msg => `${msg.type}: ${msg.content}`).join('\n')}

INSTRUCTIONS:
1. Analyze ONLY the museum's database and system records provided above
2. Provide specific, actionable insights based on the actual data
3. Suggest museum improvements that are relevant to the current visitor patterns and trends
4. If the user requests a report, suggest the most appropriate report type
5. For vague requests, offer specific options based on the available data
6. Always base recommendations on the actual museum data, not generic advice

Please provide a helpful, data-driven response and suggest relevant actions.`;
...
What would you like to explore?`;
    ...
    } else if (lowerMessage.includes('financial') || lowerMessage.toLowerCase().includes('donation') || lowerMessage.includes('money') || lowerMessage.includes('revenue')) {
